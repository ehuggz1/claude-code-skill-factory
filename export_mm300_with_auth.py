#!/usr/bin/env python3
"""
Export MM-300 to GitHub bug report with attachments (with JIRA authentication)
...
def get_jira_credentials():
    """
    Get JIRA credentials from environment or prompt user.

    To set credentials as environment variables:
    ...
    if not jira_email or not jira_token:
        print("\n⚠️  JIRA credentials not found in environment variables")
        print("\nTo download attachments, you need:")
        print("  1. JIRA account email")
        print("  2. JIRA API token (generate at https://id.atlassian.com/manage-profile/security/api-tokens)")
        print("\nOptions:")
        print("  - Enter credentials now (not saved)")
        print("  - Set environment variables: JIRA_EMAIL and JIRA_API_TOKEN")
        print("  - Skip authentication (attachments will fail to download)")
        ...
        if choice == 'y':
            ...
            print("\nSkipping authentication - attachments will not be downloaded")
            return None, None
...
def main():
    print("=" * 70)
    print("JIRA to GitHub Bug Report Exporter")
    print("Issue: MM-300 - Azure Functions Telemetry Bug")
    print("=" * 70)

    # Get JIRA credentials
    jira_email, jira_token = get_jira_credentials()

    if jira_email and jira_token:
        print(f"\n✓ Using JIRA credentials for: {jira_email}")
    else:
        print("\n⚠️  Proceeding without authentication - attachments may fail")
    ...
    # Download attachments with authentication
    print("\nDownloading attachments...")
    try:
        downloaded = reader.download_attachments(
            issue_dir,
        ...
        if downloaded:
            print(f"✓ Successfully downloaded {len(downloaded)} attachments:")
            for filepath in downloaded:
                filename = os.path.basename(filepath)
                size = os.path.getsize(filepath)
                print(f"  - {filename} ({size:,} bytes)")
        else:
            print("⚠️  No attachments were downloaded")
            if not jira_email or not jira_token:
                print("   (Authentication was not provided)")
...
"""
    ...
    print(f"✓ Generated GitHub bug report: {filepath}")

    # Summary
    print("\n" + "=" * 70)
    print("EXPORT COMPLETE")
    print("=" * 70)
    print(f"\n📁 Output directory: {issue_dir}/")
    print(f"📄 Bug report: {os.path.basename(filepath)}")
    print(f"🔗 Source JIRA: https://rightrez.atlassian.net/browse/MM-300")
